...
export const TechnicalEvaluation: React.FC<TechnicalEvaluationProps> = ({ selectedModality }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-4 bg-white dark:bg-zinc-900/80 border border-zinc-200 dark:border-zinc-800/80 rounded-2xl grid grid-cols-1 sm:grid-cols-2 gap-4 shadow-xs">
        <div>
          <label className="block text-xs font-mono text-zinc-500 dark:text-zinc-400 mb-1">Selecionar Atleta:</label>
          <select
            value={selectedStudentId}
            onChange={e => setSelectedStudentId(e.target.value)}
            className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-xl px-3 py-2 text-sm font-bold text-zinc-900 dark:text-white focus:outline-none focus:border-amber-500 cursor-pointer"
          >
            {STUDENTS_DATA.map(s => (
        ...
        <div>
          ...
          <select
            ...
            className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-xl px-3 py-2 text-sm font-bold text-amber-600 dark:text-amber-400 focus:outline-none focus:border-amber-500 cursor-pointer"
          >
      ...
      <div className="p-5 sm:p-6 bg-white dark:bg-zinc-900/80 border border-zinc-200 dark:border-zinc-800/80 rounded-2xl space-y-6 shadow-xs">
        <div className="flex items-center justify-between pb-4 border-b border-zinc-200 dark:border-zinc-800 flex-wrap gap-4">
          <div>
            <div className="flex items-center gap-2">
              <MartialIcon modalityId={activeModalityId} size={18} />
              <span className="text-xs uppercase font-mono text-amber-600 dark:text-amber-400 font-bold">
                Formulário Específico da Modalidade:
              </span>
            </div>
            <h3 className="text-lg sm:text-xl font-black text-zinc-900 dark:text-white mt-1">
              Avaliação Técnica: {MODALITIES_DATA.find(m => m.id === activeModalityId)?.name}
            </h3>
            <p className="text-xs text-zinc-500 dark:text-zinc-400">Atleta sob avaliação: {selectedStudent.name}</p>
          </div>
        ...
        <form onSubmit={handleSaveEvaluation} className="space-y-6 text-xs">
          ...
          {activeModalityId === 'bjj' && (
            <div className="space-y-4">
              <div className="p-3 bg-blue-500/10 border border-blue-500/25 rounded-xl text-blue-700 dark:text-blue-300">
                Regras BJJ: Avaliação de passagem de guarda, finalizações e indicação de até 4 graus na faixa.
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-zinc-700 dark:text-zinc-300 font-semibold mb-1">Passagem de Guarda (0 a 10)</label>
                  <input
                    type="number"
                    defaultValue={9.0}
                    step="0.5"
                ...
                <div>
                  <label className="block text-zinc-700 dark:text-zinc-300 font-semibold mb-1">Finalizações & Raspagens (0 a 10)</label>
                  <input
                    type="number"
                    defaultValue={8.5}
                    step="0.5"
                ...
                <div>
                  <label className="block text-zinc-700 dark:text-zinc-300 font-semibold mb-1">Grau Sugerido na Faixa</label>
                  <select
                    defaultValue="4"
                    className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-xl px-3 py-2 text-zinc-900 dark:text-white font-mono"
                  >
                    ...
                    <option value="4">4º Grau (Pronto para Faixa)</option>
                  </select>
                </div>
              </div>
            </div>
          ...
          {activeModalityId === 'muay_thai' && (
            <div className="space-y-4">
              <div className="p-3 bg-red-500/10 border border-red-500/25 rounded-xl text-red-700 dark:text-red-300">
                Regras Muay Thai: Acompanhamento de rounds de sparring em ringue, corte de peso e clinch.
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-zinc-700 dark:text-zinc-300 font-semibold mb-1">Rounds de Sparring Realizados</label>
                  <input
                    type="number"
                    defaultValue={8}
                    className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-xl px-3 py-2 text-zinc-900 dark:text-white font-mono"
                ...
                <div>
                  <label className="block text-zinc-700 dark:text-zinc-300 font-semibold mb-1">Pesagem Aferida no Dia (kg)</label>
                  <input
                    type="number"
                    step="0.1"
                ...
                <div>
                  <label className="block text-zinc-700 dark:text-zinc-300 font-semibold mb-1">Equipamentos de Proteção</label>
                  <select className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-xl px-3 py-2 text-zinc-900 dark:text-white">
          ...
          {activeModalityId === 'karate' && (
            <div className="space-y-4">
              <div className="p-3 bg-amber-500/10 border border-amber-500/25 rounded-xl text-amber-800 dark:text-amber-300">
                Regras Karatê: Avaliação segmentada por Kata (formas tradicionais) e Kumite (combate).
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-zinc-700 dark:text-zinc-300 font-semibold mb-1">Estilo Marcial</label>
                  <input
                    type="text"
                    defaultValue="Shotokan"
                    className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-xl px-3 py-2 text-zinc-900 dark:text-white"
                  />
                </div>
                <div>
                  <label className="block text-zinc-700 dark:text-zinc-300 font-semibold mb-1">Nota de Kata (0 a 10)</label>
                  <input
                    type="number"
                    step="0.1"
                    defaultValue={8.8}
                ...
                <div>
                  <label className="block text-zinc-700 dark:text-zinc-300 font-semibold mb-1">Nota de Kumite (0 a 10)</label>
                  <input
                    type="number"
                    step="0.1"
                    defaultValue={8.2}
          ...
          {activeModalityId === 'judo' && (
            <div className="space-y-4">
              <div className="p-3 bg-emerald-500/10 border border-emerald-500/25 rounded-xl text-emerald-800 dark:text-emerald-300">
          ...
          {activeModalityId === 'boxing' && (
            <div className="space-y-4">
              <div className="p-3 bg-purple-500/10 border border-purple-500/25 rounded-xl text-purple-800 dark:text-purple-300">
                Regras Boxe: Sem faixas. Evolução contínua por categoria de peso oficial e tempo de ringue.
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-zinc-700 dark:text-zinc-300 font-semibold mb-1">Pesagem Aferida (kg)</label>
                  <input
                    type="number"
                    step="0.1"
                    defaultValue={selectedStudent.currentWeightKg}
                ...
                <div>
                  <label className="block text-zinc-700 dark:text-zinc-300 font-semibold mb-1">Categoria de Peso</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-zinc-700 dark:text-zinc-300 font-semibold mb-1">Minutagem em Sparring (Ringue)</label>
                  <input
                    type="number"
                    defaultValue={180}
                    className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-xl px-3 py-2 text-zinc-900 dark:text-white font-mono"
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-xl p-3 text-zinc-900 dark:text-white focus:outline-none focus:border-amber-500 text-xs"
            />
          </div>

          {isSuccessMessage && (
            <div className="p-3.5 bg-emerald-500/15 border border-emerald-500/30 rounded-xl text-emerald-600 dark:text-emerald-400 text-xs flex items-center gap-2">
          ...
          <div className="pt-4 flex justify-end">
            <button
              ...
              className="px-6 py-2.5 text-white font-bold text-xs rounded-xl transition shadow-sm hover:opacity-90 cursor-pointer"
            >
