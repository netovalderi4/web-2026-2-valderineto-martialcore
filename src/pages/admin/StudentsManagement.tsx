...
export const StudentsManagement: React.FC<StudentsManagementProps> = ({ selectedModality }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-4 bg-zinc-900/80 border border-zinc-800 rounded-xl flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full bg-zinc-950 border border-zinc-800 rounded-lg pl-9 pr-4 py-2 text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-amber-500 transition"
          />
        </div>

        <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto">
          <span className="text-xs text-zinc-400 font-medium whitespace-nowrap">Financeiro:</span>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-zinc-950/70 border-b border-zinc-800 text-zinc-400 uppercase tracking-wider font-mono">
              <tr>
                ...
                <th className="py-3 px-4">Modalidades & Graduação</th>
                <th className="py-3 px-4">Evolução de Carência</th>
                  ...
                  <tr key={student.id} className="hover:bg-zinc-800/40 transition">
                    ...
                    <td className="py-3.5 px-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-zinc-400 text-[11px]">{student.email}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-3.5 px-4">
                      {student.paymentStatus === 'adimplente' && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[11px] font-semibold">
                          <CheckCircle2 className="w-3 h-3" /> Em Dia
                        </span>
                      )}
                      {student.paymentStatus === 'pendente' && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/20 text-[11px] font-semibold">
      ...
      {selectedStudent && (
        <div className="fixed inset-0 bg-black/75 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl p-6 relative">
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-2 mt-2">
                  ...
                  {selectedStudent.cbjRegistration && (
                    <span className="text-[11px] font-mono px-2 py-0.5 rounded bg-emerald-950/40 border border-emerald-500/30 text-emerald-300">
                      Filiação: {selectedStudent.cbjRegistration}
                    </span>
                  )}
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 my-6 p-4 rounded-xl bg-zinc-950/60 border border-zinc-800/80 text-xs">
              ...
              <div className="flex items-center gap-2 text-zinc-300">
                ...
                <span className="truncate">{selectedStudent.emergencyContact}</span>
              </div>
            </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        {m.isReadyForPromotion && (
                          <span className="px-2 py-0.5 rounded bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs font-bold flex items-center gap-1">
                      ...
                      <div className="grid grid-cols-2 gap-3 pt-2 text-xs font-mono">
                        <div className="p-2.5 rounded bg-zinc-900 border border-zinc-800">
                          <span className="text-zinc-400 text-[11px] block">Aulas no Grau Atual:</span>
                          <span className="text-sm font-bold text-white">
                            {m.classesAttendedInCurrentRank} / {m.classesRequired} necessárias
                          </span>
                        </div>
                        <div className="p-2.5 rounded bg-zinc-900 border border-zinc-800">
                          <span className="text-zinc-400 text-[11px] block">Carência Cumprida:</span>
                          <span className="text-sm font-bold text-white">
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 bg-black/75 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-lg p-6 relative">
            ...
            <h3 className="text-lg font-black text-white mb-1">Cadastrar Novo Atleta</h3>
            ...
            <div className="space-y-3.5 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-zinc-300 font-semibold mb-1">Data de Nascimento</label>
                  <input
                    type="date"
                    className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-amber-500"
                  />
                ...
                <div>
                  <label className="block text-zinc-300 font-semibold mb-1">Peso Atual (kg)</label>
                  <input
                    type="number"
                    step="0.1"
                    placeholder="75.0"
              ...
              <div>
                <label className="block text-zinc-300 font-semibold mb-1">Modalidade Inicial</label>
                <select className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-amber-500">
            ...
            <div className="mt-6 flex justify-end gap-2">
              ...
              <button
                ...
                className="px-4 py-2 bg-amber-500 hover:bg-amber-400 text-black rounded-lg text-xs font-bold shadow cursor-pointer"
              >
                Salvar Matrícula
              </button>
            </div>
